...
[![Build Status](https://travis-ci.org/ramiawadallah/neste.png?branch=master)](https://travis-ci.org/ramiawadallah/neste)

Neste is an implementation of the [Nested Set](http://en.wikipedia.org/wiki/Nested_set_model) pattern for [Laravel 5's](http://laravel.com/) Eloquent ORM.

> For **Laravel 4.2.x compatibility**, check the [1.0.x branch](https://github.com/ramiawadallah/neste/tree/1.0.x-stable) branch or use the latest [1.0.x tagged release](https://github.com/ramiawadallah/neste/releases).
...
Neste works with Laravel 5 onwards. You can add it to your `composer.json` file
with:

    composer requier ramiawadallah/neste

Run `composer install` to install it.

As with most Laravel 5 packages you'll then need to register the Neste
*service provider*. To do that, head over your `config/app.php` file and add
the following line into the `providers` array:

    'Neste\Providers\NesteServiceProvider',
...
run the provided generator:
    ...
    php artisan neste:install MODEL
...
configured to work with the Nested Set behaviour provided by Neste. You SHOULD
...
In order to work with Neste, you must ensure that your model class extends
`Neste\Node`.

This is the easiest it can get:

```php
class Category extends Neste\Node {
...
```php
class Dictionary extends Neste\Node {
...
You must ensure that the database table that supports your Neste models has the
following columns:
...
to use Neste with your model. Below are some examples.
...
Neste provides several methods for moving nodes around:
...
You can ask some questions to your Neste nodes:
...
Neste provides two self-referential Eloquent relations for your nodes: `parent`
and `children`.
...
Neste provides some very basic query scopes for accessing the root and leaf nodes:
...
There are several methods which Neste offers to access the ancestry/descendancy
...
Second, as **methods** which return actual `Neste\Node` instances (inside a `Collection`
...
In some situations where the hierarchy depth is huge it might be desirable to limit the number of levels of children returned (depth). You can do this in Neste by using the `limitDepth` query scope.
...
By default in Neste all results are returned sorted by the `lft` index column
...
Neste extends the default `Eloquent\Collection` class and provides the
...
Neste models fire the following events: `moving` and `moved` every time a node
...
Neste provides a simple method to provide Nested Set "scoping" which restricts
...
Neste supports for complete tree-structure rebuilding (or reindexing) via the
...
and Neste will try its best to recompute the rest.
...
Neste comes with **limited support** for soft-delete operations. What I mean
...
Because Nested Set structures usually involve a number of method calls to build a hierarchy structure (which result in several database queries), Neste provides two convenient methods which will map the supplied array of node attributes and create a hierarchy tree from them:
...
Neste provides some query scopes which may be used to extract (remove) selected nodes
...
asked questions about Neste in the [wiki](https://github.com/ramiawadallah/neste/wiki).

Feel free to browse the wiki after finishing this README:

[https://github.com/ramiawadallah/neste/wiki](https://github.com/ramiawadallah/neste/wiki)
...
Please see the [CONTRIBUTING.md](https://github.com/ramiawadallah/neste/blob/master/CONTRIBUTING.md) file for extended guidelines and/or recommendations.
